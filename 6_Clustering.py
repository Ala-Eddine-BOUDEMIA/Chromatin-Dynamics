...
def clustering_genes(
    ...
    corr_matices = [g_corr_norm, g_corr_top1000, g_corr_top76, g_corr_cv, g_corr_nrcv] 
    rand_files = sorted([f for f in g_corr_rand.iterdir() if f.is_file()])
    for path in rand_files:
        corr_matices.append(path)

    images = [g_img_clstrNorm, g_img_clstrTop, g_img_clstr76, g_img_clstrCv, g_img_clstr_nrcv]
    for i in range(len(rand_files)):
...
def clustering_samples_genes(
    ...
    images = [sg_img_clstrNorm, sg_img_clstrTop, sg_img_clstrTop76, sg_img_clstrCv, sg_img_clstrnrCv]
    for i in range(len(rand_files)):
